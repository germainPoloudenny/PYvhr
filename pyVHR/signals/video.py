...
class Video:
    ...
    def __init__(self, filename, verb=0):
        self.filename = "sampleDataset/"+str(filename[0])+"."+str(filename[1])
        self.filenameCompressed=str(filename[0])+".npz"
    ...
    def getCroppedFaces(self, detector='mtcnn', extractor='skvideo', fps=30):
        ...
        filenamez = 'cropped' + '/' + self.filenameCompressed
    ...
    def showVideo(self):
        ...
        def view_image(frame):
            ...
            if self.typeROI == 'rect':
                ...
                for coord in self.rectCoords:
                    ...
                    ax.add_patch(rect)
                #if bool:
                #    plt.savefig("/home/germain/pyVHR/pyVHR/signals/rect/"+str(frame)+".png")
            elif self.typeROI == 'skin_fix':
        ...
        #for i in range(n):
        #    view_image(i)
        interact(view_image, frame=widgets.IntSlider(min=0, max=n, step=1, value=1))
